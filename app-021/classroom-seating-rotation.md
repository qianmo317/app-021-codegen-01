...
2. **学生名单**：姓名、学号（可选，家长查询与重名区分用）、身高、视力（`none` / `front_required` / `middle_required`）、特殊需求（`hearing` / `mobility`）、学习分层 T1~T3、必须分开的对象列表、固定座位、备注；支持逐条弹窗编辑与「批量粘贴」（每行 `姓名,身高,备注` 或 `姓名,学号,身高,备注`，逗号 / 中文逗号 / 制表符分隔；允许重名但需有学号或备注区分，学号唯一）。
...
8. **导出与打印**：按周座位表 CSV、公平性统计 CSV（带 BOM，Excel 直接打开，含学号列）；打印页按周渲染 A4 纵向座位表，含讲台方向条与标记说明页脚。
9. **家长只读查询**（`src/lib/parentQuery.ts` 纯函数 + `src/pages/ParentQuery.tsx`）：选班级/周次后按姓名包含或学号检索；重名学生在候选列表中显示「学号 / 备注」供区分。结果含本周第几排第几列、区域与座位标记、八邻域周围同学（行列模式隔过道左右不算同桌、前后斜向仍列出并标「隔过道」；小组模式列同组组员）、本周特殊标记（学生照顾标签 + 硬约束未满足告警）、到目前的前 N 排次数、平均位置分与按均分升序的班级排名（并列同名次）。页面只调用 `getClass`，无任何写接口；「打印」走 `window.print()`（打印 CSS 仅保留结果卡），「存成小图」用 Canvas 2D 手绘 2× PNG 下载（`src/lib/parentCard.ts`，无第三方库）。卡片与页面均写明数据只在本机浏览器、不上传。
...
/class/:id/query           家长只读查询（选周次、按姓名/学号查单人座位与统计，可打印/存 PNG）
/query                     家长查询入口（先选班级）
```
